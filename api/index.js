...
// Use environment variable for storage (Vercel-compatible)
let endpoints = [];

try {
  const stored = process.env.ENDPOINTS;
  if (stored) {
    endpoints = JSON.parse(stored);
  }
} catch (err) {
  console.log("Could not parse stored endpoints");
}
...
function saveEndpoints(newEndpoints) {
  ...
  // In production, you'd want to persist this (database, KV store, etc.)
  // For now, it's stored in memory during the request lifecycle
}
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <div class="info-box">
            <p><strong>Note:</strong> Endpoints are stored in memory. For production, set up a database or use Vercel KV Store.</p>
          </div>
        </div>
      </div>
    </body>
...
app.post("/run", async (req, res) => {
  const results = await hitAllEndpoints();
  const eps = getEndpoints();
  res.send(`
    <!DOCTYPE html>
    <html>
...
// ---- API endpoint for running endpoints ----
